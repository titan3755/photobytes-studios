...
export default async function UserOrderPage({
  ...
  if (!session?.user) {
    redirect(`/login?callbackUrl=/dashboard/orders/${p.orderId}`);
  }

  // 2. Fetch the order and its messages
  ...
  // 3. Security Check:
  // - Ensure order exists
  // - Ensure the logged-in user is the author of this order
  if (!order) {
    redirect('/not-found');
  }
  if (order.authorId !== session.user.id) {
    redirect('/forbidden');
  }
